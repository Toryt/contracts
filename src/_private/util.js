...
exports.browserModuleLocation = browserModuleLocation;
exports.typeOf = typeOf;
exports.isPrimitive = isPrimitive;
exports.isInteger = isInteger;
exports.pre = pre;
exports.setAndFreezeProperty = setAndFreezeProperty;
exports.defineConfigurableDerivedProperty = defineConfigurableDerivedProperty;
exports.defineFrozenDerivedProperty = defineFrozenDerivedProperty;
exports.defineFrozenReadOnlyArrayProperty = defineFrozenReadOnlyArrayProperty;
exports.isFrozenOwnProperty = isFrozenOwnProperty;
exports.nrOfLines = nrOfLines;
exports.isALocationOutsideLibrary = isALocationOutsideLibrary;
exports.firstLocationOutsideLibrary = firstLocationOutsideLibrary;
exports.stackOutsideThisLibrary = stackOutsideThisLibrary;
exports.conciseConditionRepresentation = conciseConditionRepresentation;
exports.extensiveThrownRepresentation = extensiveThrownRepresentation;
/*
 ...
 */
const dirSeparator = exports.dirSeparator = /src(.)_private\1util/.exec(stackSample)[1];
 ...
 */
const atStackLocation = exports.atStackLocation = /^ {4}at (.*)(:\d+:\d+\)?| \(native\)| \(<anonymous>\))$/;
// (<anonymous>) is used in node 8.4

/**
 * Pattern that matches stack lines on Safari or Firefox, and not on node and Chrome.
 */
const atSignStackLocation = exports.atSignStackLocation = /^(.*@)?https?:\/\/[^/]*\/([^:/]*\/)*[^:/]*.js:\d+:\d+|\[native code]$/;

/**
 * Pattern that matches stack lines on the current platform.
 */
const stackLocation = exports.stackLocation = (() => {
...
function browserModuleLocation(amdModule) {
  ...
  if (location[location.length - 1].indexOf('.') >= 0) {
    // last entry is a file
    location.pop();
  }
  ...
  while (dotLocation >= 0) {
    if (dotLocation <= 3) {
      throw new Error('AMD module location "' + amdModule.uri + '" is illegal in the context of window location "' + window.location + '". ' + 'It would bring us above the root of the server. There are too many ".." elements ' + 'at the start of the uri.');
    }
...
const contractLibPath = exports.contractLibPath = pathUp(pathUp( // 2 directories up
...
function typeOf(obj) {
  ...
  if (obj === null) {
    // workaround for some weird implementations
    return 'null';
  }
...
function pre( /* Object? */self, /* Function */condition) {
...
function setAndFreezeProperty(obj, propName, value) {
  pre(function () {
...
// noinspection FunctionNamingConventionJS
function defineConfigurableDerivedProperty(prototype, propertyName, derivation) {
  pre(function () {
    return !!prototype && !isPrimitive(prototype);
  });
  pre(function () {
    return typeOf(propertyName) === 'string';
  });
  pre(function () {
    return typeOf(derivation) === 'function';
  });
...
function defineFrozenDerivedProperty(prototype, propertyName, derivation) {
  pre(function () {
    return !!prototype && !isPrimitive(prototype);
  });
  pre(function () {
...
// noinspection FunctionNamingConventionJS
function defineFrozenReadOnlyArrayProperty(prototype, propName, privatePropName) {
  pre(function () {
    return !isPrimitive(prototype);
  });
  pre(function () {
    return typeOf(propName) === 'string';
  });
  pre(function () {
    return typeOf(privatePropName) === 'string';
  });
  pre(function () {
    return propName !== privatePropName;
  });

  defineFrozenDerivedProperty(prototype, propName, function () {
    return this[privatePropName].slice();
  });
}

function isFrozenOwnProperty(obj, propName) {
  pre(function () {
    return obj !== null && obj !== undefined;
  });

  const descriptor = Object.getOwnPropertyDescriptor(obj, propName);
  return descriptor && descriptor.enumerable === true && descriptor.configurable === false && (descriptor.writable === false || typeOf(descriptor.get) === 'function' && !descriptor.set);
}

function nrOfLines(str) {
 ...
 */
function isALocationOutsideLibrary(location) {
  if (typeOf(location) !== 'string') {
    return false;
  }
  const lines = location.split(eol);
  return lines.length === 1 && stackLocation.test(lines[0]) && lines[0].indexOf(dirSeparator) >= 0 && lines[0].indexOf(this.contractLibPath) < 0;
}
 ...
 * in most consoles.
 */
function firstLocationOutsideLibrary() {
 ...
 */
function stackOutsideThisLibrary(error) {
  pre(function () {
  ...
  const result = error.stack.split(eol).splice(nrOfMessageLines) // everything after the message lines
  .reduce((acc, line) => {
...
const maxLengthOfConciseRepresentation = exports.maxLengthOfConciseRepresentation = 80;
const lengthOfEndConciseRepresentation = exports.lengthOfEndConciseRepresentation = 15;
const conciseSeparator = exports.conciseSeparator = ' … ';

/**
 * Returns a concise representation of <code>f</code> to be used in output.
 */
function conciseConditionRepresentation(prefix, f) {
  pre(function () {
 ...
 */
function extensiveThrownRepresentation(thrown) {
