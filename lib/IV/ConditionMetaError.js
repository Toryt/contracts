...
property.frozenDerived(ConditionMetaError.prototype, 'message', function () {
  // noinspection JSUnresolvedVariable
  return (
    `error occurred while evaluating ${report.conciseCondition(
      'condition',
      this.condition
    )} ` +
    `while contract function ${this.contractFunction.name} was called (${
      this.error
    })`
  )
})
