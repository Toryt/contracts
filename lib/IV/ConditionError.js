...
function ConditionError (contractFunction, condition, self, args, rawStack) {
  ...
  util.pre(this, function () { return util.isArguments(args) || Array.isArray(args) })
  util.pre(function () { return util.isAStack(rawStack) })
...
ConditionError.prototype = new ContractError(util.callerStack())
...
util.defineFrozenDerivedProperty(
  ...
  function () {
    ...
    const conditionRepresentation = util.conciseConditionRepresentation('condition', this.condition)
    // noinspection JSUnresolvedVariable
    return `${conditionRepresentation} failed while ${this.contractFunction.displayName} was called`
  }
)
util.setAndFreezeProperty(
  ...
  'getDetails',
  function () {
    const argsList = Array.prototype.map
      .call(this.args, (arg, index) => `
    ${index} (${util.type(arg)}): ${util.inspect(arg)}`)
    // noinspection JSUnresolvedVariable
    return `contract:
${this.contractFunction.contract.location}
condition:
    ${util.conciseConditionRepresentation('', this.condition)}
contract function:
${this.contractFunction.location}
this (${util.type(this.self)}):
    ${util.inspect(this.self)}    
