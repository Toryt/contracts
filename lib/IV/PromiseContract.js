...
function PromiseContract (kwargs) {
  assert.ok(kwargs)
  assert(
    !kwargs.pre || Array.isArray(kwargs.pre),
    'optional kwargs.pre is an array'
  )
  assert(
    !kwargs.post || Array.isArray(kwargs.post),
    'optional kwargs.post is an array'
  )
  assert(
    !kwargs.exception || Array.isArray(kwargs.exception),
    'optional kwargs.exception is an array'
  )
  // noinspection JSUnresolvedVariable
  assert(
    !kwargs.fastException || Array.isArray(kwargs.fastException),
    'optional kwargs.fastException is an array'
  )
  ...
  property.setAndFreeze(
    ...
    '_fastException',
    Object.freeze(
      kwargs.fastException
        ? kwargs.fastException.slice()
        : AbstractContract.mustNotHappen
    )
  )
}
...
PromiseContract.prototype.implementation = function (implFunction) {
  ...
  function contractFunction () {
    ...
    // NOTE: This assertion is added to guard against a problem with Safari on iOS in . See AbstractContract.bless.
    // noinspection JSPotentiallyInvalidConstructorUsage
    assert(
      !contractFunction.implementation.prototype ||
        contractFunction.prototype ===
          contractFunction.implementation.prototype ||
        contractFunction.prototype instanceof contractFunction.implementation,
      'contractFunction prototype must be instance of implementation function prototype'
    )
    // noinspection JSUnresolvedFunction
    PreconditionViolation.prototype.verifyAll(
      contractFunction,
      contract.pre,
      cfThis,
      arguments
    ...
    try {
      ...
      //  noinspection JSUnresolvedFunction
      PostconditionViolation.prototype.verifyAll(
        contractFunction,
        resultIsAPromise,
        cfThis,
        promiseArgs
      ...
      return promise
        .catch(rejection => {
          if (rejection instanceof ConditionError) {
      ...
      if (fastException instanceof ConditionError) {
        // necessary to report pcv and only the deepest failure clearly
        throw fastException
      }
      extendedArgs.push(fastException)
      extendedArgs.push(contractFunction.bind(cfThis))
      // noinspection JSUnresolvedFunction, JSUnresolvedVariable
      ExceptionConditionViolation.prototype.verifyAll(
        contractFunction,
        contract.fastException,
        cfThis,
        extendedArgs
      )
      throw fastException
...
PromiseContract.outcome = AbstractContract.outcome
// noinspection JSAnnotator
PromiseContract.callee = AbstractContract.callee
