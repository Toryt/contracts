...
function AbstractContract (kwargs, /* Object? */ internalLocation) {
  util.pre(() => !!kwargs)
  util.pre(() => !kwargs.pre || util.typeOf(kwargs.pre) === 'array')
  util.pre(() => !kwargs.post || util.typeOf(kwargs.post) === 'array')
  util.pre(() => !kwargs.exception || util.typeOf(kwargs.exception) === 'array')
  // MUDO util.pre(() => !internalLocation || internalLocation === AbstractContract.internalLocation)

  const self = this

  function abstract () { throw new AbstractContract.AbstractError(self) }

  const location = internalLocation || util.firstLocationOutsideLibrary()
...
AbstractContract.contractFunctionDisplayName = function (f) {
  util.pre(() => util.typeOf(f) === 'function')
...
AbstractContract.bless = function bless (contractFunction, contract, implFunction, location) {
  util.pre(() => util.typeOf(contractFunction) === 'function')
  ...
  util.pre(() => util.typeOf(implFunction) === 'function')
  // MUDO util.pre(() => location === AbstractContract.internalLocation || util.isALocationOutsideLibrary(location))
