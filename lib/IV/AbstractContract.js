...
function AbstractContract (kwargs, /* Object? */ _location) {
  ...
  property.setAndFreeze(
    self,
    '_pre',
    Object.freeze(kwargs.pre ? kwargs.pre.slice() : [])
  )
  property.setAndFreeze(
    self,
    '_post',
    Object.freeze(kwargs.post ? kwargs.post.slice() : [])
  )
  property.setAndFreeze(
    self,
    '_exception',
...
AbstractContract.bindContractFunction = function bind () {
  ...
  const boundImplementation = Function.prototype.bind.apply(
    this.implementation,
    arguments
  )
  property.frozenDerived(boundImplementation, 'name', () =>
    report.conciseCondition('bound', this.implementation)
  )
  AbstractContract.bless(
    bound,
    this.contract,
    boundImplementation,
    this.location
 ...
 */
AbstractContract.bless = function bless (
  contractFunction,
  contract,
  implFunction,
  location
  ...
  assert(
    contract instanceof AbstractContract,
    'contract is an AbstractContract'
  )
  assert.equal(typeof implFunction, 'function')
  assert(
    location === AbstractContract.internalLocation ||
      is.stackLocation(location),
  ...
  if (contractFunction !== implFunction) {
    ...
    if (implFunction.prototype && typeof implFunction.prototype === 'object') {
      ...
      assert(
        Object.getPrototypeOf(contractFunction.prototype) ===
...
function outcome (args) {
  assert.ok(args)
  assert(
    Array.isArray(args) || typeof args.length === 'number',
