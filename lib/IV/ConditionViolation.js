...
function ConditionViolation (contractFunction, condition, self, args) {
  ...
  ConditionError.call(this, contractFunction, condition, self, args, stack.raw(/* istanbul ignore next: platform dependent */ stack.skipsForEach ? 6 : 7))
}
...
property.setAndFreeze(
  ...
  function (contractFunction, conditions, self, args) {
    assert(AbstractContract.isAGeneralContractFunction(contractFunction), 'verifyAll: first argument is a contract function')
    assert.ok(conditions)
    assert(Array.isArray(conditions), 'verifyAll: conditions is an array')
    assert(conditions.every(c => typeof c === 'function'), 'verifyAll: all conditions are functions')
    assert.ok(args)
...
property.setAndFreeze(
  ...
  function (contractFunction, condition, self, args) {
    ...
    assert.equal(typeof condition, 'function', 'verify: condition is a function')
    ...
    try {
      ...
      const cme = new ConditionMetaError(contractFunction, condition, self, args, err, stack.raw(/* istanbul ignore next: platform dependent */ stack.skipsForEach ? 4 : 5))
      Object.freeze(cme)
      throw cme
    }
    if (!conditionResult) {
...
property.setAndFreeze(
  ...
  'verifyAllPromise',
  function (contractFunction, conditions, self, args) {
    assert(AbstractContract.isAGeneralContractFunction(contractFunction), 'verifyAllPromise: first argument is a contract function')
    assert.ok(conditions)
    assert(Array.isArray(conditions), 'verifyAllPromise: conditions is an array')
    assert(conditions.every(c => typeof c === 'function'), 'verifyAllPromise: all conditions are functions')
    assert.ok(args)
...
property.setAndFreeze(
  ...
  'verifyPromise',
  function (contractFunction, condition, self, args) {
    assert(AbstractContract.isAGeneralContractFunction(contractFunction), 'verifyPromise: first argument is a contract function')
    assert.equal(typeof condition, 'function', 'verifyPromise: condition is a function')
    assert(is.functionArguments(args) || Array.isArray(args), 'verifyPromise: args is arguments or array')

    function metaError (err) {
      const cme = new ConditionMetaError(contractFunction, condition, self, args, err, stack.raw(/* istanbul ignore next: platform dependent */ stack.skipsForEach ? 5 : 6))
      Object.freeze(cme)
      return cme
    }
