...
function ConditionViolation (contractFunction, condition, self, args) {
  ...
  ConditionError.call(
    ...
    args,
    stack.raw(
      /* istanbul ignore next: platform dependent */ stack.skipsForEach ? 6 : 7
    )
  )
}
 ...
 */
property.setAndFreeze(ConditionViolation.prototype, 'verifyAll', function (
  contractFunction,
  conditions,
  self,
  args
  ...
  assert(
    is.functionArguments(args) || Array.isArray(args),
    'verifyAll: args is arguments or array'
  )
 ...
 */
property.setAndFreeze(ConditionViolation.prototype, 'verify', function (
  contractFunction,
  condition,
  self,
  args
  ...
  assert(
    AbstractContract.isAGeneralContractFunction(contractFunction),
    'verify: first argument is a contract function'
  )
  assert.equal(typeof condition, 'function', 'verify: condition is a function')
  assert(
    is.functionArguments(args) || Array.isArray(args),
    'verify: args is arguments or array'
  )
  ...
  try {
    ...
    const cme = new ConditionMetaError(
      ...
      err,
      stack.raw(
        /* istanbul ignore next: platform dependent */ stack.skipsForEach
          ? 4
          : 5
      )
    )
...
property.setAndFreeze(
  ...
  function (contractFunction, conditions, self, args) {
    ...
    assert(
      ...
      'verifyAllPromise: conditions is an array'
    )
    assert(
      conditions.every(c => typeof c === 'function'),
    ...
    assert(
      is.functionArguments(args) || Array.isArray(args),
      'verifyAllPromise: args is arguments or array'
    )

    return Promise.all(
      conditions.map(condition =>
        this.verifyPromise(contractFunction, condition, self, args)
      )
    )
  }
)
 ...
 */
property.setAndFreeze(ConditionViolation.prototype, 'verifyPromise', function (
  contractFunction,
  condition,
  self,
  args
  ...
  assert.equal(
    typeof condition,
    'function',
    'verifyPromise: condition is a function'
  )
  assert(
    is.functionArguments(args) || Array.isArray(args),
    'verifyPromise: args is arguments or array'
  )
