...
import {primitive, functionArguments} from "./is"
import {strictEqual, ok} from "assert";

import {stack as stackEOL} from "./eol";

function safeToString (s: any): string {
  try {
    return String(s);
  } catch (ignore) {
...
export function value (v: any): string {
 ...
 */
export function extensiveThrown (thrown: any): string {
  const thrownString: string = value(thrown);
  let stack: string = thrown && thrown.stack;
  if (!stack) {
    return thrownString;
  }
