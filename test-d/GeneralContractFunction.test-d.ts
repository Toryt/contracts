...
// TODO this is still using the overloaded CallableFunction methods
expectNotAssignable<GeneralContractFunction<(b: number) => string, SomeError>>(aCallableGeneralContractFunctionB.bind(someObject, anA))
expectAssignable<(b: number) => string>(aCallableGeneralContractFunctionB.bind(someObject, anA))
expectAssignable<string>(aCallableGeneralContractFunctionB.bind(someObject, anA)(aB))
// TODO this is still using the overloaded CallableFunction methods
expectNotAssignable<GeneralContractFunction<() => string, SomeError>>(aCallableGeneralContractFunctionB.bind(someObject, anA, aB))
expectAssignable<() => string>(aCallableGeneralContractFunctionB.bind(someObject, anA, aB))
expectAssignable<string>(aCallableGeneralContractFunctionB.bind(someObject, anA, aB)())
// `prototype` is defined with type `any` in `lib.es5.d.ts`, and there is nothing we can do about that … (***)
...
expectType<AFunction1['bind'] & CallableBind<AFunction1, SomeError>>(aCallableGeneralContractFunctionC.bind)
// TODO this is still using the overloaded CallableFunction methods
expectNotAssignable<GeneralContractFunction<(a:string, b: number) => string, SomeError>>(aCallableGeneralContractFunctionC.bind(someObject))
...
// TODO this is still using the overloaded CallableFunction methods
expectNotAssignable<GeneralContractFunction<(b: number) => string, SomeError>>(aCallableGeneralContractFunctionC.bind(someObject, anA))
expectAssignable<(b: number) => string>(aCallableGeneralContractFunctionC.bind(someObject, anA))
expectAssignable<string>(aCallableGeneralContractFunctionC.bind(someObject, anA)(aB))
// TODO this is still using the overloaded CallableFunction methods
expectNotAssignable<GeneralContractFunction<() => string, SomeError>>(aCallableGeneralContractFunctionC.bind(someObject, anA, aB))
expectAssignable<() => string>(aCallableGeneralContractFunctionC.bind(someObject, anA, aB))
expectAssignable<string>(aCallableGeneralContractFunctionC.bind(someObject, anA, aB)())
// `prototype` is defined with type `any` in `lib.es5.d.ts`, and there is nothing we can do about that … (***)
...
expectType<typeof ANewableFunction.bind & NewableBind<typeof ANewableFunction, SomeError>>(aNewableGeneralContractFunctionB.bind)
expectAssignable<NewableBind<typeof ANewableFunction, SomeError>>(aNewableGeneralContractFunctionB.bind)
...
expectNotAssignable<GeneralContractFunction<new (b: number) => ANewableGeneralContractFunction, SomeError>>(boundB2)
...
// TODO this is still using the overloaded NewableFunction methods
expectNotAssignable<GeneralContractFunction<new () => ANewableGeneralContractFunction, SomeError>>(boundB3)
...
expectType<typeof ANewableFunction.bind & NewableBind<typeof ANewableFunction, SomeError>>(aNewableGeneralContractFunctionC.bind)
/* NOTE tsd says that for the next line, expectType does not work. But the output _is_ the same? */
expectAssignable<NewableBind<typeof ANewableFunction, SomeError>>(aNewableGeneralContractFunctionC.bind)
...
// TODO this is still using the overloaded NewableFunction methods
expectNotAssignable<GeneralContractFunction<new (b: number) => ANewableGeneralContractFunction, SomeError>>(boundC2)
...
// TODO this is still using the overloaded NewableFunction methods
expectNotAssignable<GeneralContractFunction<new () => ANewableGeneralContractFunction, SomeError>>(boundC3)
